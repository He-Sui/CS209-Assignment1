        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                Course course = new Course(info[0], info[1], new Date(info[2]),
                        info[3], info[4], info[5], Integer.parseInt(info[6]),
    ...
    public Map<String, Integer> getPtcpCountByInstAndSubject() {
        return courses.stream()
                .collect(Collectors.groupingBy(
                        x -> x.getInstitution().concat("-").concat(x.getSubject()),
    ...
    public Map<String, List<List<String>>> getCourseListOfInstructor() {
        return courses.stream()
                .flatMap(course -> course.getInstructors().stream()
                        .map(instructor -> new AbstractMap.SimpleEntry<>(instructor, course)))
                .collect(Collectors.groupingBy(
                        Map.Entry::getKey,
    ...
    public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
        ...
        return courses.stream()
                .map(course -> new AbstractMap.SimpleEntry<>(
                        course, Math.pow(age - averageAge.get(course.getNumber()), 2)
                        + Math.pow(gender * 100 - averageMalePercent.get(course.getNumber()), 2)
                        + Math.pow(isBachelorOrHigher * 100
                        - averagePercentDegree.get(course.getNumber()), 2)))
                .sorted(Map.Entry.comparingByValue())
                .map(e -> new AbstractMap.SimpleEntry<>(e.getKey().getNumber(), e.getValue()))
                .collect(Collectors.toMap(AbstractMap.SimpleEntry::getKey,
                        AbstractMap.SimpleEntry::getValue, (v1, v2) -> v1)).entrySet().stream()
                .sorted(Map.Entry.comparingByValue())
